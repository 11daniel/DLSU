...
def extract_info(file_hash, virus_total_data, id_number):
    hash_type = detect_hash_algorithm(file_hash)
    
    if "error" in virus_total_data:
        return {"ID": id_number, "Hash": file_hash, "Algorithm": hash_type, "Detection Count": "Not Found", "Name1": "N/A", "Name2": "N/A", "Name3": "N/A", "Verdict": "Unknown"}
...
def worker(queue):
    while not queue.empty():
        file_hash, id_number = queue.get()
        print(f"Checking hash: {file_hash}...")
        vt_data = query_virustotal(file_hash)
        result = extract_info(file_hash, vt_data, id_number)
