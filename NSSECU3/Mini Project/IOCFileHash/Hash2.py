...
SLEEP_TIME = 16
OUTPUT_FILE = "C:/Users/nicos/OneDrive/Documents/DLSU/Term 8/NSSECU3/IOCFileHash/virustotal_resultsF.csv"

def detect_hash_algorithm(file_hash):
...
def extract_info(file_hash, virus_total_data):
    hash_type = detect_hash_algorithm(file_hash)
    
    if "error" in virus_total_data:
        return {"Hash": file_hash, "Algorithm": hash_type, "Detection Count": "Not Found", "Name1": "N/A", "Name2": "N/A", "Name3": "N/A", "Verdict": "Unknown"}
    ...
    for engine, result in malicious_results.items():
        ...
        if detection_name and detection_name not in detected_names:
            detected_names.append(detection_name)
        if len(detected_names) >= 3:
            break  

    while len(detected_names) < 3:
        detected_names.append("None")
    ...
    return {
        ...
        "Creation Time": convert_timestamp(data.get("creation_time")),
        "Signature Date": data.get("signature_date", "N/A"),
        "First Seen In The Wild": convert_timestamp(data.get("first_seen_itw_date")),
        "First Submission": convert_timestamp(data.get("first_submission_date")),
        "Last Submission": convert_timestamp(data.get("last_submission_date")),
        "Last Analysis": convert_timestamp(data.get("last_analysis_date")),
        "Name1": detected_names[0],
        "Name2": detected_names[1],
        "Name3": detected_names[2],
