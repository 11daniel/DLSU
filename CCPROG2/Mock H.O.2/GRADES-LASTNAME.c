...
{
 	//Have the index be nStudents and assign all the parameters to thee end
...
{
	int i, x = 0;
	//run through the entire struct array of students and if their practice score is 0 and midtermscore < param_score, add them to the new list
	for(i = 0; i < nStudents; i++)
	{
		if(STUDENTS[i].practice_score == 0 && STUDENTS[i].midterm_score < param_score)
		{
			newList[x] = STUDENTS[i];
			x++;
		}
	}
	return x;
