...
class RuleMiner(object):
    ...
    def get_support(self, data, itemset):
        ...
        # function.
        itemset_support = data[itemset].all(axis=1)
        return itemset_support.sum()
    ...
    def get_confidence(self, data, rule):
        ...
        # class.
        x_support = self.get_support(data, rule[0])
        x_and_y_support = self.get_support(data, rule[0] + rule[1])

        return x_and_y_support / x_support if x_support != 0 else 0
