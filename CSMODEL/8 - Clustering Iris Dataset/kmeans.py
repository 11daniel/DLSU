...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            data_point_ind = distances.min(axis=1).idxmax()

            # Append the selected data point to the set of centroids.
            point = data.iloc[data_point_ind, self.start_var:self.end_var]
            point = point.set_axis(self.centroids.columns).to_frame().T
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            ...
            return np.sqrt(((point1 - point2) ** 2).astype(float).sum(axis=1))
        else:
    ...
    def adjust_centroids(self, data, groups):
        ...
        # TODO: Complete this function.
        selected_cols = data.select_dtypes(include=np.number)
        grouped_data = pd.concat([selected_cols, groups.rename('group')], axis=1)
